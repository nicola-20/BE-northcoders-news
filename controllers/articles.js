  ...
  Article.findOneAndUpdate(
    ...
    .then(article => {
      if (!vote) {
        return Promise.reject({
          status: 400,
          msg: `"${Object.keys(req.query)[0]}" is not a valid query`
        });
      }
      if (voteChange === 0) {
        return Promise.reject({
          status: 400,
          msg: `"${vote}" is not a valid value for this query`
        });
      }
      res.send({ article });
    })
  ...
  Topic.findOne({ slug: topic_slug })
    .then(topic => {
      if (!topic) {
        return Promise.reject({
          status: 400,
          msg: `"${topic_slug}" is not a valid topic`
        });
      } else {
        ...
        .then(article => {
          if (!article) {
            return Promise.reject({
              status: 400,
              msg: `"${req.body}" is not a valid article`
            });
          }
